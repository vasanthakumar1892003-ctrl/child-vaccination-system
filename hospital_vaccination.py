...
<title>Childhood Vaccination Schedule - Hospital</title>
...
.navbar-brand i {{
  margin-right: 10px; color: #e9d5ff; font-size: 1.5rem; animation: bounce 2s infinite;
}}
@keyframes bounce {{
...
.vaccination-header {{
  ...
  padding-bottom: 20px; border-bottom: 3px solid #7c3aed;
}}
...
.vaccination-table {{
  ...
  overflow: hidden; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);
}}
.vaccination-table thead {{
  background: linear-gradient(135deg, #083344, #22d3ee);
}}
.vaccination-table th {{
  color: white; font-weight: 600; padding: 20px 15px;
  text-align: left; font-size: 1em; text-transform: uppercase; letter-spacing: 1px;
}}
.vaccination-table tbody tr {{ transition: all 0.3s ease; background: white; }}
.vaccination-table tbody tr:nth-child(odd) {{ background: #faf5ff; }}
.vaccination-table tbody tr:hover {{
  background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%);
}}
.vaccination-table td {{
  padding: 18px 15px; border-bottom: 1px solid #e9d5ff;
  vertical-align: middle; font-size: 0.95em; color: #4a5568;
}}
.age-milestone {{ border-bottom: 2px solid #c4b5fd !important; }}
.vaccine-list {{ line-height: 1.6; color: #5b21b6; font-weight: 500; }}
.disease-list {{ line-height: 1.6; color: #6b21a8; }}
.vaccination-footer {{
  margin-top: 30px; padding: 20px;
  background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%);
  border-radius: 10px; text-align: center; color: #5b21b6; font-weight: 500;
}}
...
<div class="container-fluid">
  <div class="row">
    <div class="col-lg-2 col-md-3 sidebar p-0" id="sidebar">
      ...
      <a href="hospital_vaccination.py?hospital_id={hospital_id}" class="sidebar-link active">
